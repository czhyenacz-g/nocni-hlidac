import { CameraDefinition } from "@/game/core/types";
import { useCopy } from "@/game/i18n/useTranslation";

interface CameraMonitorTileProps {
  camera: CameraDefinition;
  /** Viz GameState.cameraDamage.disabledCameraIds — jen malý červený "OFFLINE" štítek (overview beztak nikdy neukazuje živý obraz, viz zadání "nesmí být vidět aktuální pozice monstra" — v mřížce to tak nehrozí). */
  offline?: boolean;
  onClick: () => void;
}

// Jeden monitor v overview mřížce (viz CameraMonitorGrid.tsx). Ukazuje jen
// štítek + statický šum, ŽÁDNÝ živý obraz — jinak by šlo sledovat všechny
// kamery najednou zdarma, viz GameState.cameraViewMode. Skutečný obraz je
// jen v detailu (CameraDetailView.tsx) po kliknutí sem.
export default function CameraMonitorTile({ camera, offline, onClick }: CameraMonitorTileProps) {
  const COPY = useCopy();
  const cameraCopy = COPY.cameras[camera.id];
  return (
    <button
      className="pixel-button pixel-screen-static camera-monitor-tile tap-target group relative h-20 lg:h-28 w-full flex flex-col items-center justify-center gap-1 px-2 text-center"
      onClick={onClick}
      aria-label={`${cameraCopy.label}${COPY.game.cameraExpandAriaSuffix}${offline ? COPY.game.cameraOfflineAriaSuffix : ""}`}
    >
      <span className="text-[9px] lg:text-[10px] text-gray-400 leading-tight">{cameraCopy.label}</span>
      {offline && <span className="text-[8px] lg:text-[9px] text-red-500 tracking-wide">OFFLINE</span>}
      <span className="text-[8px] lg:text-[9px] text-gray-600">⤢</span>
      {/* Popis kamery (viz COPY.cameras, stejné jako v detailu —
          CameraView.tsx) — schovaný, dokud hráč nenajede myší na monitor, ať
          nezaplácá malý štítek, ale poradí před kliknutím. */}
      {cameraCopy.description && (
        <span className="pointer-events-none absolute left-1/2 top-full z-20 mt-1 w-36 -translate-x-1/2 rounded border border-gray-700 bg-black/90 px-2 py-1 text-[9px] leading-tight text-gray-400 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
          {cameraCopy.description}
        </span>
      )}
    </button>
  );
}
